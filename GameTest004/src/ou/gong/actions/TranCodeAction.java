...
//	       server_name = new String(request.getParameter("server_name").getBytes("iso-8859-1"), "utf-8"); 
			server_name = new String(request.getParameter("server_name").trim());
			if(server_name.equals("0") || server_name == null) {
				request.setAttribute("msg", "请选择服务器") ;
		        return SUCCESS ;
			}
	       System.out.println(server_name);
	       game_name = (String)request.getParameter("game").trim() ;
	       System.out.println(game_name);
	       issue = Integer.parseInt((String)request.getParameter("issue").trim()) ;
	       station_id = Integer.parseInt((String)request.getParameter("station_id").trim()) ;
	       station_cert = Integer.parseInt((String)request.getParameter("station_cert").trim()) ;
	       type = Integer.parseInt((String)request.getParameter("type").trim()) ;
	       stat = Integer.parseInt((String)request.getParameter("stat").trim()) ;
	       tsn = Integer.parseInt((String)request.getParameter("tsn").trim()) ;
	       money = Float.parseFloat((String)request.getParameter("money").trim()) ;
...
//				ts.setWager_money(money) ;//设置缴款金额为10000 
				...
				msg = new TestCaseDao().tc877805(tf, sf, sheet_name) ;
			}
