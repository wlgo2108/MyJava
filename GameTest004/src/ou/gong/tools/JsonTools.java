...
//		MakeJson mJson = new MakeJson();
		...
		String sheetName = "GSK3";
		List<TestCase> tList = new ExcelToDB().toDb(pro_id, game_id, tsn, filename, sheetName);
		String jSonStr = JsonTools.listToJson(tList);
		try {
			jsonTools.writeToJson("c:\\logs\\" + sheetName + ".json", jSonStr);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		
		List<TestCase> tcList = JsonTools.jsonToTestCase("c:\\logs\\" + sheetName + ".json") ;
//		System.out.println(tcList.size());
		
		for(TestCase tc : tcList) {
			
			System.out.println(tc.getComment()) ;
		}
